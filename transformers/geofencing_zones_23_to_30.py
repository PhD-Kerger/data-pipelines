...
class GeofencingZonesTransformer:
    def __init__(
    ...
    def run_transformer(self):
        ...
        self.logger.info(
            f"Starting GeofencingZones transformation for operator {self.operator}."
        )
        ...
        for filename in tqdm.tqdm(
            ...
            if filename.endswith(".tar.gz") or filename.endswith(".tgz"):
                ...
                with tarfile.open(input_path, "r:gz") as tar_in:
                    ...
                    # check that json is not empty and it does not contain `"httpCode"`
                    if not json_content.strip() or b'"httpCode"' in json_content:
                        self.logger.warning(
                            f"Skipping '{filename}': JSON file is empty for operator {self.operator}."
                        )
                        continue
                    ...
                    with tarfile.open(output_path, "w:gz") as tar_out:
                        ...
                        data["version"] = "3.0"
                        # update geofencing_zones[].features[].properties.name
                        if "geofencing_zones" in data["data"]:
                            if "features" in data["data"]["geofencing_zones"]:
                                for feature in data["data"]["geofencing_zones"][
                                    "features"
                                ]:
                                    if (
                                        "properties" in feature
                                        and "name" in feature["properties"]
                                    ):
                                        name_text = feature["properties"]["name"]
                                        feature["properties"]["name"] = [
                                            {"text": name_text, "language": "en"}
                                        ]

                        # add global_rules[]
                        if "global_rules" not in data["data"]:
                            data["data"]["global_rules"] = []
