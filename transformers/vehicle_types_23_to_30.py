...
class VehicleTypesTransformer:
    ...
    def run_transformer(self):
        ...
        self.logger.info(
            f"Starting VehicleTypes transformation for operator {self.operator}."
        )
        ...
        for filename in tqdm.tqdm(
            ...
            if filename.endswith(".tar.gz") or filename.endswith(".tgz"):
                ...
                with tarfile.open(input_path, "r:gz") as tar_in:
                    ...
                    with tarfile.open(output_path, "w:gz") as tar_out:
                        ...
                        if "last_updated" in data:
                            ...
                            # Convert to 2023-07-17T13:34:13+00:00 format
                        ...
                        # update vehicle_types[].name, make, eco_labels
                        if "vehicle_types" in data["data"]:
                            for vtype in data["data"]["vehicle_types"]:
                                if "name" in vtype:
                                    name_text = vtype["name"]
                                    vtype["name"] = [
                                        {"text": name_text, "language": "en"}
                                    ]
                                if "make" in vtype:
                                    make_text = vtype["make"]
                                    vtype["make"] = [
                                        {"text": make_text, "language": "en"}
                                    ]
                                if "eco_label" in vtype:
                                    eco_labels = vtype["eco_label"]
                                    vtype["eco_labels"] = eco_labels
                                    del vtype["eco_label"]
                ...
                if not output_written and all_files_copied == False:
                    self.logger.warning(
